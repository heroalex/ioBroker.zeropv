...
class ConfigValidator {
    ...
    static validateAndNormalize(config, logger) {
        ...
        // Validate price config (optional feature)
        if (config.priceSourceObject !== undefined) {
            if (typeof config.priceSourceObject !== 'string' || !config.priceSourceObject.trim()) {
                result.errors.push('Invalid price source object: must be a non-empty string');
                result.isValid = false;
            }
        }

        // Validate price threshold (default to 0 if invalid)
        if (config.priceThreshold === undefined || isNaN(config.priceThreshold)) {
            config.priceThreshold = 0;
            logger.warn('Invalid price threshold, defaulting to 0 €/kWh');
        }
